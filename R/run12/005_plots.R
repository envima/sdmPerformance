...
source(paste0("R/",nameRun,"/functions/check.R"))
...
if(!dir.exists(paste0("images/",nameRun,"/assumptions"))) dir.create(paste0("images/",nameRun,"/assumptions"), recursive=T)
...
for (m in metric_names) {
  ...
  for(method_plot in c("PAA", "PA", "PBG")){
    ...
    #data2=data2%>%dplyr::slice_sample(n=4999)
    fit <- dem_reg(x = m,
                   y = "trueCor_scaled",
                   data = data2,
                   error.ratio = 0.05,
    ...
    assumptions <- check2(fit)
    ggsave(assumptions, filename = paste0("images/",nameRun,"/assumptions/",m, "_",method_plot,".png"), dpi = 300)
    # Ergebnisse anzeigen
    # summary(fit)
    rm(assumptions)
    ...
    intercept <- fit$model$coef[1][[1]]
    slope <- fit$model$coef[2][[1]]
    
    #fit_summary<- fit$groupsummary
    
    slope_pValue <- fit$model$p.value[2][[1]]
    intercept_pValue <- fit$model$p.value[1][[1]]
    
    # Compute predicted values along the RMA line
    data2$y_deming <- intercept + slope * data2[[m]]
    ...
    label_text <- paste0(
     ...
     # "\nR² = ", round(metrics_df$r_squared_rma, 2),
