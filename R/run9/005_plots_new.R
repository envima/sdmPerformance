  ...
  for(method_plot in c("PAA", "PA", "PBG")){
    ...
    metrics_df <- data2 %>%
      ...
      summarise(
        ...
        rmse2 = sqrt(mean((trueCor_scaled - y_rma)^2, na.rm = TRUE)),
...
for (m in metric_names) {
  ...
  for(method_plot in c("PAA", "PA", "PBG")){
    for(prevalence_plot in unique(data$class)){
      ...
      # check assumption 1: Is relationship between the data linear
      # H0: 
      # Fit a linear model
      lm_model <- lm(paste0("trueCor_scaled ~ ",m), data = data2)
      
      # Fit a non-linear model (e.g. quadratic)
      nls_model <- nls(paste("trueCor_scaled ~ a * ",m,"^2 + b * ",m," + c"), data = data2, start = c(a = 1, b = 1, c = 1))
      
      # Compare the models using anova
      an <- anova(nls_model, lm_model)
      if (an$`Pr(>F)`[2] < 0.05) break
      ...
      #PBequi_summary$metric <- m
      #PBequi_summary$method <- method_plot
      #PBequi_summary$prevalence <- prevalence_plot
      #all_results[[paste0(m, "_",method_plot,"_",prevalence_plot)]]  <- PBequi_summary
...
for (m in metric_names) {
  ...
  for(method_plot in c("PAA", "PA", "PBG")){
    for(point_plot in c("40", "80", "120", "160", "200", "400")){
      ...
      #  PBequi_summary$metric <- m
      # PBequi_summary$method <- method_plot
      #  PBequi_summary$point <- point_plot
      #  all_results[[paste0(m, "_",method_plot,"_",point_plot)]]  <- PBequi_summary
