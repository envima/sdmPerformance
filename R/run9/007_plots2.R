...
dataSummary=readRDS(paste0("images/",nameRun,"/resultPlotsPoint/resultsPoint.RDS"))
...
dataSummary%>%dplyr::group_by(method)%>% dplyr::summarise(median = median(Slope, na.rm = TRUE), mean = mean(Slope, na.rm = TRUE))
dataSummary%>%dplyr::group_by(method)%>% dplyr::summarise(median = median(intercept, na.rm = TRUE), mean= mean(intercept, na.rm = TRUE))
