...
source(paste0("R/run3/functions/scale_metric.R"))
source("R/run4/functions/weighted_mean_po.R")
...
metric_names <- c("AUC", "COR", "Spec", "Sens", "Kappa", "SBI_m",
                  "PCC", "TSS", "PRG", "trueCor",  "FM","MCC","SEDI", "ORSS","BS")
...
data <- data %>%
  rowwise() %>%  # Make operations work row by row
  mutate(
    omissionRate1 = mean(c(1 - omissionRate), na.rm = TRUE),
    omissionRatePBG1 =mean(c(1 - omissionRatePBG), na.rm = TRUE),
    #omissionRateFpb = mean(c(1 - omissionRatePO), na.rm = TRUE),
    weightedMean = weighted_extreme_mean(TopQ, 1-omissionRate, alpha=4),
    #omissionRate1 = weighted_extreme_mean(TopQ, 1-omissionRate, alpha=4),
    #omissionRateMean = mean(c(omissionRate1, omissionRateFpb), na.rm = TRUE),
    weightedMeanBoyce = weighted_extreme_mean(TopQ, SBI_m_scaled, alpha=4),
    weightedMeanPBG = weighted_extreme_mean(TopQ, omissionRatePBG1, alpha=4),
   # weightedMeanPO = weighted_extreme_mean(TopQ, omissionRatePO, alpha=4)
  ) %>%
...
metric_names <- c( "AUC_scaled", "COR_scaled", "Spec_scaled", "Sens_scaled", "Kappa_scaled", "Fbp","omissionRate","SBI_tp",          
                  "SBI_cr","SBI_bs","SBI_ps","SBI_ad","SBI_m", "omissionRate", "SBI_m_scaled",     "weightedMean", "weightedMeanBoyce", "weightedMeanPO",
                   "PCC_scaled", "TSS_scaled", "PRG_scaled", "AACBS", "TopQ","MAE", "BS","FM_scaled","MCC_scaled","SEDI_scaled", "ORSS_scaled")
