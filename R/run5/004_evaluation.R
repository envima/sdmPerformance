...
mclapply(1:nrow(df), function(i){
  ...
  print(i)
  if(!file.exists(paste0("data/",nameRun,"/results/",as.character(df$species[i]),"_",as.character(df$size[i]),"_",as.character(df$model[i]),"_testData",df$testData[i],"_points",as.character(df$points[i]),"_replicates",df$replicates[i],"_noPresencePoints",df$NoPO[i],".RDS"))){
    ...
    if(nrow(test%>%dplyr::filter(Real==1))<100) return(NULL)
    ...
    if(isTRUE(terra::global(pred, fun = function(x) all(is.na(x)))[[1]])) return(NULL)
      ...
      presenceDownsampled=test%>%dplyr::filter(Real==1)%>%dplyr::sample_n(df$NoPO[i])
      
      result=performanceStabilityIndex( x = NA,
                                        prediction=pred,
                                        presence = presenceDownsampled,
      ...
      saveRDS(result, paste0("data/",nameRun,"/results/",as.character(df$species[i]),"_",as.character(df$size[i]),"_",as.character(df$model[i]),"_testData",df$testData[i],"_points",as.character(df$points[i]),"_replicates",df$replicates[i],"_noPresencePoints",df$NoPO[i],".RDS"))
