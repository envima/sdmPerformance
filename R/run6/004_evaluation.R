...
df=expand.grid(size=as.character(c("KNNDM","random","block1","block2","clusters")) ,
               ...
               testData = 1:6,
               NoReplicatesMetrics=seq(1,100,1))
...
source(paste0("R/run4/functions/performanceStabilityIndexReplicates.R"))
...
mclapply(1:nrow(df), function(i){
  ...
  print(i)
  if(!file.exists(paste0("data/",nameRun,"/results/",as.character(df$species[i]),"_",as.character(df$size[i]),"_",as.character(df$model[i]),"_testData",df$testData[i],"_points",as.character(df$points[i]),"_replicates",df$replicates[i],"_NoReplicatesMetrics",df$NoReplicatesMetrics[i],".RDS"))){
    ...
    if(isTRUE(terra::global(pred, fun = function(x) all(is.na(x)))[[1]])) return(NULL)
      ...
      result=performanceStabilityIndex( x = NA,
                                        ...
                                        noPointsTesting = NA,
                                        replicates=df$NoReplicatesMetrics[i])
      ...
      result$NoReplicatesMetrics <- df$NoReplicatesMetrics[i]
      
      #result$randomEffects <- randomEffects
      #result$autocorrRange <- autocorrRange
      
      rm(pred,realDistribution,test,vs, autocorrRange, randomEffects,presenceDownsampled);gc()
      
      if(!dir.exists(paste0("data/",nameRun,"/results"))) dir.create(paste0("data/",nameRun,"/results"), recursive=T)
      saveRDS(result, paste0("data/",nameRun,"/results/",as.character(df$species[i]),"_",as.character(df$size[i]),"_",as.character(df$model[i]),"_testData",df$testData[i],"_points",as.character(df$points[i]),"_replicates",df$replicates[i],"_NoReplicatesMetrics",df$NoReplicatesMetrics[i],".RDS"))
