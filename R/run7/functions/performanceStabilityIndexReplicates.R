...
indexCalculation <- function(inputDF, stability) {
  ...
  brierScore=brier_score(actual=inputDF$observed, predicted_probs = inputDF$predicted)
 ...
 AACBS=AACBS(predicted=inputDF$predicted, observed=inputDF$observed)
  ...
  # all metrics in one df
  result=data.frame(metric=metric,Fbp=FBP,AACBS=AACBS,brierScore=brierScore,FM=FM,MCC=MCC, SEDI=SEDI,ORSS=ORSS,AUC=AUC, COR=COR, Spec,Sens,Kappa,PCC, TSS,stability=stability, PRG=PRG, MAE=MAE, BIAS=BIAS, noPresencePoints=nrow(inputDF[inputDF$observed ==1,]))
  return(result)
}
